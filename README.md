...
| `POST` | `/api/player/{did}/play` | body 支持 `keyword` / `song` / `folder` / `artist` / `playlist` / `favorites` / `index` |
...
四种播放模式在「播放」页的下拉里也能切。`顺序播放` 与 `随机播放` 播到列表末尾
就停下（界面显示「列表已播放完毕」，队列还留着，点队列里的某首可以接着放），
只有 `全部循环` 会回头重放——这是自动续播时唯一会绕回开头的模式。
